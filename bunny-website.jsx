...
const generateConnectDots = (seed) => {
  const points = [
    { x: 200, y: 50, n: 1 },   // top of ear
    { x: 180, y: 80, n: 2 },
    { x: 170, y: 120, n: 3 },
    { x: 160, y: 150, n: 4 },  // ear base
    { x: 140, y: 170, n: 5 },  // head left
    { x: 130, y: 200, n: 6 },
    { x: 130, y: 240, n: 7 },
    { x: 140, y: 280, n: 8 },
    { x: 160, y: 310, n: 9 },  // chin
    { x: 200, y: 330, n: 10 }, // bottom chin
    { x: 240, y: 310, n: 11 },
    { x: 260, y: 280, n: 12 },
    { x: 270, y: 240, n: 13 },
    { x: 270, y: 200, n: 14 },
    { x: 260, y: 170, n: 15 }, // head right
    { x: 240, y: 150, n: 16 }, // ear base
    { x: 230, y: 120, n: 17 },
    { x: 220, y: 80, n: 18 },
    { x: 200, y: 50, n: 19 },  // back to top
  ];
  ...
  return `
    <svg viewBox="0 0 400 500" xmlns="http://www.w3.org/2000/svg">
      ...
      <!-- Light guide lines (optional hint) -->
      <path class="hint" d="M ${points.map(p => `${p.x} ${p.y}`).join(' L ')}" />
...
const Navigation = ({ activeSection, setActiveSection }) => {
  const sections = [
    { id: 'joke', label: '😄 Daily Joke', icon: '😄' },
    { id: 'famous', label: '⭐ Famous Bunnies', icon: '⭐' },
    { id: 'story', label: '📖 Stories', icon: '📖' },
    { id: 'map', label: '🗺️ Habitat Map', icon: '🗺️' },
    { id: 'gallery', label: '📸 Gallery', icon: '📸' },
    { id: 'video', label: '🎬 Daily Video', icon: '🎬' },
    { id: 'adopt', label: '🏠 Adopt', icon: '🏠' },
    { id: 'craft', label: '✂️ Daily Craft', icon: '✂️' },
  ];
      ...
      <Card style={{ maxWidth: '800px', margin: '0 auto' }}>
        <div style={{
          ...
          {literatureStories.map((s, i) => (
            <button
              key={i}
              onClick={() => { setCurrentStory(i); window.speechSynthesis.cancel(); setIsReading(false); }}
              style={{
                padding: '10px 20px',
                border: 'none',
                borderRadius: '20px',
...
const CraftSection = () => {
  ...
  const [craftType, setCraftType] = useState(currentCraftType);
...
// ==================== MAIN APP ====================
export default function BunnyWebsite() {
